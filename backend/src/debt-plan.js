...
const itemSchema = new mongoose.Schema({
  ...
  status: { type: String, enum: ["ACTIVE", "OVERDUE", "NEGOTIATED", "PAID"] },
...
export function validateDebt(body) {
  ...
  const status = oneOf(body.status || "ACTIVE", ["ACTIVE", "OVERDUE", "NEGOTIATED", "PAID"], "Status");
  ...
  return { name: requiredText(body.name, "Nome", 160), description: optionalText(body.description, "", 300),
...
export function negotiateDebt(debt, body, userId) {
  ...
  return { ...debt, currentBalance: negotiatedBalance, minimumPayment: newInstallment, interestRateMonthly: newInterestRate,
    interestRateAnnual: null, dueDate: newDueDate, remainingInstallments: newInstallments,
    status: negotiatedBalance === 0 ? "PAID" : "NEGOTIATED", balanceOverride: true,
...
export function registerDebtPlanRoutes(app, deps) {
  ...
  for (const action of ["negotiate", "mark-paid", "update"]) app.post(base + `/debts/:debtId/${action}`, auth, wrap((req, res) => edit(req, res, (plan, ctx) => {
