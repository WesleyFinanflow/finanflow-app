...
export default function App() {
  ...
  return <main className="finanflow-real"><aside className="real-sidebar"><div className="brand-row"><div className="brand-icon">♙</div><div><strong>FinanFlow</strong><span>Modo individual</span></div></div><nav>{menu.map((item) => <button key={item} className={activeMenu === item ? "active" : ""} onClick={() => setActiveMenu(item)}>{item}</button>)}</nav><button className="couple-button" onClick={() => setActiveMenu("Casal")}>Ir para casal</button><button className="danger-button" onClick={resetSpaceData}>Zerar dados</button></aside><section className="real-content"><Hero firstName={firstName} />{activeMenu === "Início" && <Inicio summary={summary} hasData={hasData} setActiveMenu={setActiveMenu} buyForm={buyForm} setBuyForm={setBuyForm} reserve={reserve} transactions={transactions} />}{activeMenu === "Lançamentos" && <Lancamentos txForm={txForm} setTxForm={setTxForm} addTransaction={addTransaction} transactions={transactions} accounts={accounts} editingTransactionId={editingTransactionId} setEditingTransactionId={setEditingTransactionId} editTransaction={editTransaction} deleteTransaction={deleteTransaction} />}{activeMenu === "Contas" && <Contas accounts={accounts} setAccounts={setAccounts} accountForm={accountForm} setAccountForm={setAccountForm} addAccount={addAccount} updateAccount={updateAccount} deleteAccount={deleteAccount} firstName={firstName} />}{activeMenu === "Planejamento" && <Planejamento summary={summary} buyForm={buyForm} setBuyForm={setBuyForm} />}{activeMenu === "Configurações" && <Config reserve={reserve} setReserve={setReserve} firstName={firstName} coupleSpace={coupleSpace} setActiveMenu={setActiveMenu} />}{activeMenu === "Casal" && <Casal coupleSpace={coupleSpace} coupleInvite={coupleInvite} createCouple={createCouple} firstName={firstName} />}<button className="logout-fixed" onClick={logout}>Sair</button>{message && <div className="floating-message">{message}</div>}</section></main>;
}
