...
function Hero({ firstName, user, coupleSpace, summary, hasData, activeMenu, hideValues, toggleValuePrivacy }) {
  ...
  const hasNegativeFreeBalance = hasData && summary.free < 0;
  ...
  return (
    <section className={`hero ${activeMenu === "Configurações" ? "settings-hero" : ""}`}>
      ...
      <div className={`balance-focus ${hasNegativeFreeBalance ? "negative" : ""}`}>
        ...
        <small>{hasNegativeFreeBalance ? "Atenção: compromissos acima do saldo disponível." : "Protegido pela sua reserva financeira."}</small>
      </div>
    </section>
  );
}
  ...
  return (
    <>
      <section className="stats-grid">
        ...
        <StatCard title="Total já pago" value={hasData ? money(summary.paidExpenses + summary.paidDebt) : "Aguardando dados"} text="Despesas e dívidas quitadas no mês" tone="spent" />
        <StatCard title="Livre seguro" value={hasData ? money(summary.free) : "Aguardando dados"} text={hasNegativeFreeBalance ? `Faltam ${money(Math.abs(summary.free))} após proteger ${money(reserve)}` : `Limite protegido: ${money(reserve)}`} tone={hasNegativeFreeBalance ? "red" : "blue"} />
      </section>
...
function ReceiptPhotoFlow({ onReady, onBack }) {
  const cameraRef = useRef(null), galleryRef = useRef(null), pdfRef = useRef(null);
  ...
  return <section className="panel receipt-source"><div className="panel-head receipt-source-head"><h2>Enviar comprovante</h2><button type="button" className="icon-close" aria-label="Fechar envio de comprovante" onClick={onBack}><X size={20}/></button></div><div className="receipt-source-grid"><button type="button" onClick={()=>cameraRef.current?.click()}><Camera size={27}/><span><strong>Tirar foto</strong><small>Abra a câmera do celular</small></span></button><button type="button" onClick={()=>galleryRef.current?.click()}><FileText size={27}/><span><strong>Escolher imagem</strong><small>Abra a galeria do celular</small></span></button><button type="button" onClick={()=>pdfRef.current?.click()}><FileDown size={27}/><span><strong>Escolher PDF</strong><small>Abra os arquivos do celular</small></span></button></div>{error&&<div className="status-box">{error}</div>}<input ref={cameraRef} hidden type="file" accept="image/*" capture="environment" onChange={e=>readFile(e.target.files?.[0])}/><input ref={galleryRef} hidden type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>readFile(e.target.files?.[0])}/><input ref={pdfRef} hidden type="file" accept="application/pdf,.pdf" onChange={e=>readFile(e.target.files?.[0])}/></section>;
}
...
function Lancamentos({ txForm, setTxForm, addTransaction, transactions, accounts, editingTransactionId, setEditingTransactionId, editTransaction, deleteTransaction, loading, formOpen, setFormOpen, selectedMonthKey, setSelectedMonthKey, activeMode }) {
  ...
  return (
    <section className={`transactions-layout ${formOpen ? "with-form" : ""}`}>
      ...
      {formOpen && !editingTransactionId && entryMode === "choice" && <section className="panel launch-choice"><div className="panel-head"><div><span className="eyebrow">Lançar movimentação</span><h2>Como você quer registrar?</h2></div><button type="button" className="icon-close" aria-label="Fechar lançamento" onClick={resetForm}><X size={18}/></button></div><div className="launch-choice-grid"><button type="button" className="featured" onClick={()=>setEntryMode("photo")}><Camera size={27}/><span><strong>Enviar comprovante</strong><small>Tire uma foto ou escolha uma imagem ou PDF</small></span><b>›</b></button><button type="button" onClick={()=>setEntryMode("manual")}><FileText size={27}/><span><strong>Lançamento manual</strong><small>Preencha os dados manualmente</small></span><b>›</b></button></div></section>}
      {formOpen && !editingTransactionId && entryMode === "photo" && <ReceiptPhotoFlow onBack={()=>setEntryMode("choice")} onReady={(data, preview, notice="")=>{setReceiptPreview(preview);setReceiptNotice(notice);setTxForm({...createTransactionForm("despesa",data.date||today),type:data.type||"despesa",description:data.description||"",amount:data.amount?String(data.amount):"",date:data.date||today,time:data.time||"",category:data.category||"",paymentMethod:data.paymentMethod||"Não informado",notes:"",status:"pago",origin:"photo",ocrMerchant:data.description||"",ocrConfidence:data.confidence});setEntryMode("manual");}}/>}
  ...
  return (
    <section className="settings-premium-grid">
      ...
      <section className="settings-card settings-protection">
        ...
        {activeMode === "couple" ? <>
          <h3>Proteção total do casal: {money(reserve)}</h3>
          <p>Este valor é a soma da proteção individual das duas pessoas. Para alterar a sua parte, volte ao Meu espaço e ajuste o limite individual.</p>
        </> : <>
...
function StatCard({ title, value, text, tone }) {
  const icons = { cyan: balanceWalletIcon, green: incomeWalletIcon, yellow: commitmentsCalendarIcon, spent: commitmentsCalendarIcon, blue: safeShieldIcon, red: safeShieldIcon };
  return (
