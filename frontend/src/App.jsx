  ...
  return (
    <main className="finanflow-real">
      ...
      <section className="real-content">
        <Hero firstName={firstName} coupleSpace={activeCoupleSpace} />
...
function Hero({ firstName, coupleSpace }) {
  const isCouple = Boolean(coupleSpace);
  return <section className="hero-real"><div><span className="eyebrow">{isCouple ? "Controle financeiro compartilhado" : "Controle financeiro individual"}</span><h1>{isCouple ? coupleSpace.name : firstName}</h1><p>{isCouple ? "Você está no espaço do casal. Cadastre os dados compartilhados para calcular o saldo livre." : "Você está no modo individual. Cadastre seus dados; o modo casal só começa depois do convite."}</p></div><div className="month-card"><span>Mês analisado</span><strong>{new Date().toLocaleDateString("pt-BR", { month: "long", year: "numeric" })}</strong></div></section>;
}
