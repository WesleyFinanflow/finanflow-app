...
export default function DebtPlanPage({ api, spaceId, currentUserId, back, embedded = false, monthlyOnly = false, openMonthly }) {
  ...
  return <section className="debt-plan">
    ...
    {!data ? !error && <div className="panel" role="status">Carregando seu plano…</div> : <>
      {!monthlyView && <><div className="dp-grid"><Kpi label="Total em dívidas" value={money(data.summary.remaining)} /><Kpi label="Parcelas mensais" value={money(data.summary.monthlyCommitment)} /><Kpi label="Dívidas ativas" value={data.summary.active} /><Kpi label="Atrasadas" value={data.summary.overdue} tone={data.summary.overdue ? "danger" : ""} /><Kpi label="Negativadas" value={data.summary.negativeListed} tone={data.summary.negativeListed ? "danger" : ""} /><Kpi label="Protestadas" value={data.summary.protested} tone={data.summary.protested ? "danger" : ""} /><Kpi label="Dívida ativa" value={data.summary.activeDebt} tone={data.summary.activeDebt ? "danger" : ""} /><Kpi label="Dívidas antigas" value={data.summary.oldDebt} /><Kpi label="Valor sugerido pelo FinanFlow" value={money(data.suggestedPayment)} tone="positive" /><Kpi label="Valor que eu posso pagar" value={data.userPayment == null ? "Não informado" : money(data.userPayment)} /></div>
      <section className="panel"><h2>Situação das dívidas</h2><p>Os totais são marcadores independentes: uma mesma dívida pode estar atrasada e negativada.</p><div className="dp-grid"><Kpi label="Em dia" value={data.debts.filter(d=>d.status==="ACTIVE"&&!d.isOverdue).length}/><Kpi label="Negociadas" value={data.debts.filter(d=>d.status==="NEGOTIATED").length}/><Kpi label="Quitadas" value={data.summary.paidCount}/></div></section></>}
      ...
      {!monthlyView && <section className="panel"><div className="dp-section-head"><h2>Defina seu plano</h2><button className="ghost-button" disabled={busy} onClick={() => mutate("/debt-plan/recalculate")}><RefreshCw size={16} />Atualizar dados</button></div>
      ...
      {monthlyView && <section className="panel"><div className="dp-section-head"><h2>Resumo do plano mensal</h2><button className="ghost-button" disabled={busy} onClick={() => mutate("/debt-plan/recalculate")}><RefreshCw size={16} />Recalcular plano</button></div><p>Estratégia escolhida: <strong>{strategies[data.strategy]}</strong>.</p><div className="dp-grid"><Kpi label="Você pode pagar por mês" value={data.userPayment == null ? "Não informado" : money(data.userPayment)} /><Kpi label="Previsão com seu valor" value={dateLabel(data.comparison.userEndDate)} /><Kpi label="Sugestão do FinanFlow" value={money(data.suggestedPayment)} tone="positive" /><Kpi label="Previsão com a sugestão" value={dateLabel(data.comparison.suggestedEndDate)} tone="positive" /></div>{data.comparison.userMonths != null && data.comparison.suggestedMonths != null && data.comparison.userMonths > data.comparison.suggestedMonths && <p className="dp-notice">Seguindo a sugestão do FinanFlow, a quitação ocorre cerca de {data.comparison.userMonths - data.comparison.suggestedMonths} mês(es) antes.</p>}</section>}
      ...
      {!monthlyView && <section className="panel"><div className="dp-section-head"><h2>Ordem de ataque</h2><label>Situação<select value={filter} onChange={e => setFilter(e.target.value)}><option value="all">Todas</option><option value="ACTIVE">Em dia</option><option value="OVERDUE">Atrasadas</option><option value="NEGATIVE_LISTED">Negativadas</option><option value="PROTESTED">Protestadas</option><option value="ACTIVE_DEBT">Dívida ativa</option><option value="NEGOTIATED">Negociadas</option><option value="PAID">Quitadas</option><option value="old">Dívidas antigas</option>{data.spaceType === "couple"&&<option value="shared">Compartilhadas</option>}{data.spaceType === "couple"&&data.members.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label><label>Tipo<select value={typeFilter} onChange={e=>setTypeFilter(e.target.value)}><option value="all">Todos os tipos</option>{Object.entries(data.catalog.types).map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label></div>
        <div className="dp-debts">{data.debts.filter(d => (filter === "all" || filter === "old" ? filter !== "old" || d.isOldDebt : filter === "shared" ? d.shared : Object.keys(statuses).includes(filter) ? d.status === filter || (filter === "OVERDUE" && d.isOverdue) || (filter === "NEGATIVE_LISTED" && d.isNegativeListed) || (filter === "PROTESTED" && d.isProtested) || (filter === "ACTIVE_DEBT" && d.isActiveDebt) : !d.shared && d.ownerUserId === filter) && (typeFilter === "all" || d.debtType === typeFilter)).map(debt => {
          ...
          return <article key={debt.id} className={`dp-debt ${debt.status === "OVERDUE" ? "overdue" : ""}`}>
            ...
            <p>Tipo: {data.catalog.types[debt.debtType] || "Outras"} · Previsão: {debt.status === "PAID" ? "Quitada" : dateLabel(debt.payoffDate)} · Prioridade {debt.priorityScore >= 50 ? "alta" : debt.priorityScore >= 25 ? "média" : "normal"}</p><div className="dp-chips">{debt.isOverdue&&<span>Em atraso</span>}{debt.isNegativeListed&&<span>CPF com restrição</span>}{debt.isProtested&&<span>Protesto ativo</span>}{debt.isActiveDebt&&<span>Dívida ativa</span>}{debt.isOldDebt&&<span>Dívida antiga</span>}</div>
