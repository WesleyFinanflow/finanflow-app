...
export default function DebtPlanPage({ api, spaceId, currentUserId, back }) {
  ...
  const [modal, setModal] = useState(null), [filter, setFilter] = useState("all"), [typeFilter, setTypeFilter] = useState("all"), [payment, setPayment] = useState(""), [monthsVisible, setMonthsVisible] = useState(6), [monthlyView, setMonthlyView] = useState(false);
  ...
  return <section className="debt-plan">
    ...
    {!data ? !error && <div className="panel" role="status">Carregando seu plano…</div> : <>
      ...
      <section className="panel"><div className="dp-section-head"><h2>{monthlyView ? "Plano mensal para quitar dívidas" : "Defina seu plano"}</h2><button className="ghost-button" disabled={busy} onClick={() => mutate("/debt-plan/recalculate")}><RefreshCw size={16} />{monthlyView ? "Recalcular plano" : "Atualizar dados"}</button></div>
        ...
        {!monthlyView && <button onClick={async()=>{if(await mutate("/debt-plan",{strategy:data.strategy,monthlyPaymentAmount:payment===""?null:payment,generate:true},"PUT"))setMonthlyView(true)}}>{data.generated ? "Ver plano mensal" : "Gerar plano"}</button>}
      </section>
