  ...
  const totals = transactions.reduce(
    (result, transaction) => {
      ...
      if (transaction.status === "pago") {
        if (transaction.type === "receita") { result.totalReceived += amount; result.balanceDelta += amount; if (inPeriod) result.received += amount; }
        if (transaction.type === "despesa") { result.totalPaidExpenses += amount; result.balanceDelta -= amount; if (inPeriod) result.paidExpenses += amount; }
        if (transaction.type === "divida") { result.totalPaidDebt += amount; result.balanceDelta -= amount; if (inPeriod) result.paidDebt += amount; }
    ...
    { income: 0, received: 0, totalReceived: 0, expenses: 0, paidExpenses: 0, totalPaidExpenses: 0, debt: 0, goals: 0, paidDebt: 0, totalPaidDebt: 0, savedGoals: 0, balanceDelta: 0 }
  );
